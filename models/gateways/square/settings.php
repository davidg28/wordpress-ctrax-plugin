...
use C_Trax_Integration\Includes\Encryption;
...
class Settings extends \WC_Settings_API {
	...
	public function __construct() {
		$this->plugin_id = C_TRAX_INTEGRATION_ACTION . '_';
		$this->id        = C_TRAX_INTEGRATION_ACTION;
	...
	public function init_form_fields() {
		...
		if ( $this->is_connected() ) {
			...
			$general_description = sprintf(
				...
				__( 'Sync your products and inventory and also accept credit and debit card payments at checkout. %1$sClick here%2$s to configure payments.', C_TRAX_INTEGRATION_PREFIX ),
				'<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=checkout&section=' . $this->plugin_id ) ) . '">',
				'</a>'
			);

		} else {

			$general_description = __( 'Connect with Square to start syncing your products and inventory and also accept credit and debit card payments at checkout.', C_TRAX_INTEGRATION_PREFIX );
		}
		...
		$fields['enable_sandbox'] = array(
			'title'       => __( 'Enable Sandbox Mode', C_TRAX_INTEGRATION_PREFIX ),
			'label'       => '<span>' . __( 'Enable to set the plugin in sandbox mode.', C_TRAX_INTEGRATION_PREFIX ) . '</span>',
			'type'        => 'checkbox',
			'description' => __( 'After enabling you’ll see a new Sandbox settings section with two fields; Sandbox Application ID & Sandbox Access Token.', C_TRAX_INTEGRATION_PREFIX ),
		);

		$fields['sandbox_settings'] = array(
			'type'        => 'title',
			'title'       => __( 'Sandbox settings', C_TRAX_INTEGRATION_PREFIX ),
			'id'          => C_TRAX_INTEGRATION_ACTION . '_wc_square_sandbox_settings',
			'description' => sprintf(
				// translators: Placeholders: %1$s - URL.
				__( 'Sandbox details can be created at: %s', C_TRAX_INTEGRATION_PREFIX ),
		...
		$fields['sandbox_application_id'] = array(
			'type'        => 'input',
			'title'       => __( 'Sandbox Application ID', C_TRAX_INTEGRATION_PREFIX ),
			'class'       => C_TRAX_INTEGRATION_ACTION . '_wc_square_sandbox_settings',
			'description' => __( 'Application ID for the Sandbox Application, see the details in the My Applications section.', C_TRAX_INTEGRATION_PREFIX ),
		);

		$fields['sandbox_token'] = array(
			'type'        => 'input',
			'title'       => __( 'Sandbox Access Token', C_TRAX_INTEGRATION_PREFIX ),
			'class'       => C_TRAX_INTEGRATION_ACTION . '_wc_square_sandbox_settings',
			'description' => __( 'Access Token for the Sandbox Test Account, see the details in the Sandbox Test Account section. Make sure you use the correct Sandbox Access Token for your application. For a given Sandbox Test Account, each Authorized Application is assigned a different Access Token.', C_TRAX_INTEGRATION_PREFIX ),
		);
		...
		if ( $this->is_connected() ) {
			...
			$fields[ $this->get_environment() . '_location_id' ] = array(
				'title'       => __( 'Business location', C_TRAX_INTEGRATION_PREFIX ),
				'type'        => 'select',
				'class'       => 'wc-enhanced-select',
				'description' => sprintf(
					/* translators: Placeholders: %1$s - <strong> tag, %2$s - </strong> tag, %3$s - <a> tag, %4$s - </a> tag */
					__( 'Select a location to link to this site. Only %1$sactive%2$s %3$slocations%4$s that support credit card processing in Square can be linked.', C_TRAX_INTEGRATION_PREFIX ),
					'<strong>',
					'</strong>',
			...
			$fields['system_of_record'] = array(
				'title'       => __( 'Product system of record', C_TRAX_INTEGRATION_PREFIX ),
				'type'        => 'select',
				'class'       => 'wc-enhanced-select',
				'description' => sprintf(
					/* translators: Placeholders: %1$s - <strong> tag, %2$s - </strong> tag, %3$s - <a> tag, %4$s - </a> tag */
					__( 'Choose where you will update data for synced products. Inventory in Square is %1$salways%2$s checked for adjustments when sync is enabled. %3$sClick here%4$s to read more about choosing a system of record.', C_TRAX_INTEGRATION_PREFIX ),
					'<strong>',
					'</strong>',
				...
				'options'     => array(
					self::SYSTEM_OF_RECORD_DISABLED    => __( 'Do not sync product data', C_TRAX_INTEGRATION_PREFIX ),
					self::SYSTEM_OF_RECORD_SQUARE      => __( 'Square', C_TRAX_INTEGRATION_PREFIX ),
					self::SYSTEM_OF_RECORD_WOOCOMMERCE => __( 'WooCommerce', C_TRAX_INTEGRATION_PREFIX ),
				),
				'default'     => 'disabled',
			);

			$fields['enable_inventory_sync'] = array(
				'title'       => __( 'Sync inventory', C_TRAX_INTEGRATION_PREFIX ),
				'label'       => '<span>' . __( 'Enable to sync product inventory with Square', C_TRAX_INTEGRATION_PREFIX ) . '</span>',
				'type'        => 'checkbox',
				'description' => __( 'Inventory is fetched from Square periodically and updated in WooCommerce', C_TRAX_INTEGRATION_PREFIX ),
			);

			$fields['hide_missing_products'] = array(
				'title'       => __( 'Handle missing products', C_TRAX_INTEGRATION_PREFIX ),
				'label'       => __( 'Hide synced products when not found in Square', C_TRAX_INTEGRATION_PREFIX ),
				'type'        => 'checkbox',
				'description' => __( 'Products not found in Square will be hidden in the WooCommerce product catalog.', C_TRAX_INTEGRATION_PREFIX ),
			);

			$fields['import_products'] = array(
				'title'    => __( 'Import Products', C_TRAX_INTEGRATION_PREFIX ),
				'type'     => 'import_products',
				'desc_tip' => __( 'Run an import to create new products in this WooCommerce store for each new product created in Square that has a unique SKU not existing in here. Needs to be run each time new items are created in Square.', C_TRAX_INTEGRATION_PREFIX ),
			);
		}
		...
		if ( ! ( $this->is_sandbox() && ! $this->is_connected() ) ) {
			$fields = array_merge(
				...
				array(
					'connect' => array(
						'title'    => __( 'Connection', C_TRAX_INTEGRATION_PREFIX ),
						'type'     => 'connect',
						'desc_tip' => '',
					),
				)
		...
		$fields = array_merge(
			...
			array(
				'debug_logging_enabled' => array(
					'title' => __( 'Enable Logging', C_TRAX_INTEGRATION_PREFIX ),
					'type'  => 'checkbox',
					'label' => sprintf(
						/* translators: Placeholders: %1$s - <a> tag, %2$s - </a> tag */
						__( 'Log debug messages to the %1$sWooCommerce status log%2$s', C_TRAX_INTEGRATION_PREFIX ),
	...
	public function get_form_fields() {
		...
		if ( isset( $_POST[C_TRAX_INTEGRATION_ACTION . '_wc_square_environment'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing

			$environment = 'yes' === $this->settings['enable_sandbox'] ? 'sandbox' : 'production';

			if ( $environment !== $_POST[C_TRAX_INTEGRATION_ACTION . '_wc_square_environment'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
				return $fields;
			}
		}
		...
		if ( did_action( C_TRAX_INTEGRATION_ACTION . '_wc_square_initialized' ) && $this->is_admin_settings_screen() && ! empty( $location_id_field_key ) ) {

			$locations = array(
				'' => __( 'Please choose a location', C_TRAX_INTEGRATION_PREFIX ),
			);
		...
		<tr valign="top">
			...
			<td class="forminp">
				<a href='#' class='button js-import-square-products'>
					<?php echo esc_html__( 'Import all products from Square', C_TRAX_INTEGRATION_PREFIX ); ?>
				</a>
			</td>
		</tr>
		<?php
	...
	public function update_refresh_token( $token ) {
		...
		if ( ! empty( $token ) ) {
			...
			if ( Encryption::is_encryption_supported() ) {
...
// TODO: Add log ability
		...
		update_option( C_TRAX_INTEGRATION_ACTION . '_wc_square_refresh_tokens', $refresh_tokens );
	}
...
//					$this->get_plugin()->log( 'Could not encrypt access token. ' . $exception->getMessage() );
		...
		update_option( C_TRAX_INTEGRATION_ACTION . '_wc_square_access_tokens', $access_tokens );
	}
	...
	public function clear_refresh_tokens() {
		delete_option( C_TRAX_INTEGRATION_ACTION . '_wc_square_refresh_tokens' );
	}
	...
	public function clear_access_tokens() {
		...
		delete_option( C_TRAX_INTEGRATION_ACTION . '_wc_square_access_tokens' );
	}
	...
	public function is_inventory_sync_enabled() {
		...
		return (bool) apply_filters( C_TRAX_INTEGRATION_ACTION . '_wc_square_inventory_sync_enabled', 'yes' === get_option( 'woocommerce_manage_stock' ) && $this->is_product_sync_enabled() && 'yes' === $this->get_option( 'enable_inventory_sync' ) );
	}
	...
	public function get_location_id() {
		...
		if ( empty( $location_id ) ) {
			$square_db_version = get_option( C_TRAX_INTEGRATION_ACTION . '_wc_' . $this->id . '_version' );
	...
	public function get_locations() {
		...
		if ( ! $this->is_admin_settings_screen() ) {
			...
			$this->locations = get_transient( C_TRAX_INTEGRATION_ACTION . '_wc_square_locations' );
		}

		if ( ! is_array( $this->locations ) && did_action( C_TRAX_INTEGRATION_ACTION . '_wc_square_initialized' ) ) {
			...
			try {
				...
				set_transient( C_TRAX_INTEGRATION_ACTION . '_wc_square_locations', $this->locations, HOUR_IN_SECONDS );
	...
	public function get_system_of_record_name() {
		...
		switch ( $this->get_system_of_record() ) {
			...
			case 'square':
				$sor = __( 'Square', C_TRAX_INTEGRATION_PREFIX );
				break;
			case 'woocommerce':
				$sor = __( 'WooCommerce', C_TRAX_INTEGRATION_PREFIX );
				break;
			default:
				$sor = '';
				break;
	...
	public function get_refresh_token() {
		...
		if ( empty( $this->refresh_token ) ) {
			...
			if ( $token && Encryption::is_encryption_supported() ) {
		 ...
		 */
		return apply_filters( C_TRAX_INTEGRATION_ACTION . '_wc_square_refresh_token', $this->refresh_token );
	}
	...
	public function get_access_token() {
		 ...
		 */
		return apply_filters( C_TRAX_INTEGRATION_ACTION . '_wc_square_access_token', $this->access_token );
	}
	...
	public function get_access_tokens() {
		return (array) get_option( C_TRAX_INTEGRATION_ACTION . '_wc_square_access_tokens', array() );
	}
	...
	public function get_refresh_tokens() {
		return (array) get_option( C_TRAX_INTEGRATION_ACTION . '_wc_square_refresh_tokens', array() );
	}
	...
	public function get_environment() {
		$sandboxed = ( defined( 'C_TRAX_INTEGRATION_SQUARE_SANDBOX' ) && C_TRAX_INTEGRATION_SQUARE_SANDBOX ) || $this->is_sandbox_setting_enabled();
		return $sandboxed ? 'sandbox' : 'production';
	}
	...
	public function is_admin_settings_screen() {
		return isset( $_GET['page'], $_GET['tab'] ) && C_TRAX_INTEGRATION_PREFIX . '-wc-settings' === $_GET['page'] && $this->id === $_GET['tab']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	}
}
